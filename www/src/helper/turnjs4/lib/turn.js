...
turnMethods = {
	...
	addPage: function(element, page) {
		...
		if (page) {
			...
			if (page==lastPage)
				...
				throw error('Page "'+page+'" cannot be inserted');
	...
	zoom: function(newZoom) {
		...
		if (typeof(newZoom)=='number') {
			...
			if (newZoom<0.001 || newZoom>100)
				throw error(newZoom+ ' is not a value for zoom');
	...
	_makeFlip: function(page) {
		...
		if (!data.pages[page] && data.pagePlace[page]==page) {
			...
			if (data.pageObjs[page].hasClass('hard'))
				...
				data.pageObjs[page].addClass('sheet');

				if (single)
					corner = 'all';
				else
					corner = (odd) ? 'forward' : 'backward';
			}
	...
	range: function(page) {
		...
		var remainingPages, left, right, view,
			...
			if (page<1 || page>data.totalPages)
				throw error('"'+page+'" is not a page for range');
	...
	removePage: function(page) {
		...
		if (page<1 || page>data.totalPages)
			throw error('The page '+ page + ' doesn\'t exist');
	...
	display: function(display) {
		...
		if (display) {
			...
			if ($.inArray(display, displays)==-1)
				throw error('"'+display + '" is not a value for display');
	...
	peel: function(corner, animate) {
		...
		animate = (typeof(animate)=='undefined') ? true : animate===true;
		...
		if (corner===false) {
			...
			if (data.display=='single') {
				...
				var page = (corner.indexOf('l')!=-1) ? view[0] : view[1],
					topCorner = corner.indexOf('t')!=-1;
...
flipMethods = {
	...
	_eventEnd: function() {
		...
		if (!data.disabled && corner) {
			...
			this.trigger(event, [data.point || corner]);
	...
	peel: function (corner, animate) {
		...
		if (corner) {
			...
			if ($.inArray(corner, flipMethods._cAllowed.call(this))==-1)
				throw error('Corner '+corner+' is not permitted');
